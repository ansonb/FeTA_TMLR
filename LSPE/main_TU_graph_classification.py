...
"""
from nets.TU_graph_classification.load_net import gnn_model # import all GNNS
...
def train_val_pipeline(MODEL_NAME, dataset, params, net_params, dirs):
    ...
    if MODEL_NAME in ['SAN', 'GraphiT', 'Spectra']:
    ...
    model = gnn_model(MODEL_NAME, net_params, edge_features_present=False, node_features_dim=trainset[0][0].ndata['attr'].shape[1])
    ...
    from train.train_tu_graph_classification import train_epoch_sparse as train_epoch, evaluate_network_sparse as evaluate_network
    ...
    try:
        with tqdm(range(params['epochs'])) as t:
            for epoch in t:
                ...
                epoch_val_loss, epoch_val_auc, __ = evaluate_network(model, device, val_loader, epoch)
                epoch_test_loss, epoch_test_auc, __ = evaluate_network(model, device, test_loader, epoch)
                del __
    ...
    test_loss_lapeig, test_auc, g_outs_test = evaluate_network(model, device, test_loader, epoch)
    train_loss_lapeig, train_auc, g_outs_train = evaluate_network(model, device, train_loader, epoch)
