...
class GraphiTSpectraNet(nn.Module):
    def __init__(self, net_params):
        ...
        if self.pe_init == 'rand_walk':
            # LSPE
            if net_params.get('last_layer_filter', False):
                ...
                self.layers = nn.ModuleList([ GraphiT_Spectra_LSPE_Layer(gamma, GT_hidden_dim, GT_hidden_dim, GT_n_heads, full_graph,
                                                                dropout, self.layer_norm, self.batch_norm, self.residual, self.adaptive_edge_PE) for _ in range(GT_layers-1) ])
            self.layers.append(GraphiT_Spectra_LSPE_Layer(gamma, GT_hidden_dim, GT_out_dim, GT_n_heads, full_graph,
                                                     dropout, self.layer_norm, self.batch_norm, self.residual, self.adaptive_edge_PE))
        else: 
            # TODO: Implement for NoPE
