...
"""
def view_model_param(LPE, net_params, node_features_dim=1):
    model = gnn_model(LPE, net_params, edge_features_present=False, node_features_dim=node_features_dim)
    total_param = 0
    print("MODEL DETAILS:\n")
...
def train_val_pipeline(MODEL_NAME, dataset, params, net_params, dirs):
    ...
    if net_params['LPE'] in ['edge', 'node', 'spectral_node']:
        st = time.time()
    ...
    net_params['total_param'] = view_model_param(net_params['LPE'], net_params, node_features_dim=trainset[0][0].ndata['attr'].shape[1])    
    ...
    model = gnn_model(net_params['LPE'], net_params, edge_features_present=False, node_features_dim=trainset[0][0].ndata['attr'].shape[1])
...
def main():    
    ...
    if net_params['LPE'] not in ['node', 'edge', 'none', 'spectral_node']:
