...
class MultiHeadAttentionLayer(nn.Module):
    def __init__(self, gamma, in_dim, out_dim, num_heads, full_graph, use_bias, edge_features_present=True):
        super().__init__()
...
class GraphTransformerLayerSpectra(nn.Module):
    """
        Param: 
    """
    def __init__(self, gamma, in_dim, out_dim, num_heads, full_graph, dropout=0.0, layer_norm=False, batch_norm=True, residual=True, use_bias=False, filter_order=4, edge_features_present=True):
        super().__init__()
        ...
        self.attention = MultiHeadAttentionLayer(gamma, in_dim, out_dim//num_heads, num_heads, full_graph, use_bias, edge_features_present=edge_features_present)
    ...
    def forward(self, g, h, e):
        ...
        h_attn_out, attn = self.attention(g, h, e)
        ...
        # num_nodes = g.num_nodes() #DEBUG
        # filter_coeff = torch.ones(filter_coeff.shape).to(utils.DEVICE) #DEBUG
        ...
        # new_g.ndata['x'] = h_attn_out.permute([1,0,2]).reshape([-1,h_attn_out.shape[-1]])
        # h_filt_out = self.mpgnn_layer2(new_g).to(utils.DEVICE)
        # # DEBUG
        ...
        # TODO: use self.residual condition for skip connection
        h = h + h_filt_out
    ...
    def get_filter_coeff(self, g, attn_w):
        ...
        # num_nodes_org = g.num_nodes()
        # num_nodes = num_nodes_org*self.num_heads
        # edges = g.edges()
        # num_edges_org = edges[0].shape[0]
        # offset = torch.arange(self.num_heads, device=edges[0].device)*num_nodes_org
        # offset = offset.unsqueeze(1).repeat([1,num_edges_org]).reshape([-1,])
        # edges_s = edges[0].repeat([self.num_heads,])
        # edges_s = edges_s+offset
        # edges_d = edges[1].repeat([self.num_heads,])
        # edges_d = edges_d+offset
        # # new_edges = (edges_s,edges_d)
        # # real_ids = torch.nonzero(attn)
        # new_g = dgl.DGLGraph().to(g.device)
        # new_g.add_nodes(num_nodes)
        # new_g.add_edges(edges_s.to(torch.int64), edges_d.to(torch.int64))
        # new_g.batch_num_nodes = g.batch_num_nodes().repeat([self.num_heads])
        # new_g.batch_num_edges = g.batch_num_edges().repeat([self.num_heads])
        # # new_g = dgl.DGLGraph(attn[real_ids])
