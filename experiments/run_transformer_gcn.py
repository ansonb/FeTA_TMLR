...
from transformer.models import DiffGraphTransformer, GraphTransformer, DiffGraphTransformerGCN
...
def main():
    ...
    val_dset = GraphDataset_v2(datasets.ZINC(data_path, subset=False, split='val'), n_tags, degree=True)
    ...
    if args.pos_enc is not None:
        ...
        if pos_encoding_method is not None:
            ...
            pos_cache_path = '{}/zinc_{}_{}_{}.pkl'.format(pos_cache_dir,args.pos_enc, args.normalization, pos_encoding_params_str)
    ...
    if args.pos_enc is not None:
        model = DiffGraphTransformerGCN(in_size=input_size,
                                     nb_class=1,
        ...
        model = GraphTransformerGCN(in_size=input_size,
                                 nb_class=1,
                                 d_model=args.dim_hidden,
                                 dim_feedforward=2*args.dim_hidden,
                                 dropout=args.dropout,
    ...
    test_dset = GraphDataset_v2(datasets.ZINC(data_path, subset=False, split='test'), n_tags, degree=True)
