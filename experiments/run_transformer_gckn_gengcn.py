...
from transformer.models import DiffGraphTransformer, GraphTransformer, DiffGraphTransformerGenGCN
...
def main():
    ...
    train_dset = datasets.ZINC(data_path, subset=False, split='train')
    val_dset = datasets.ZINC(data_path, subset=False, split='val')
    test_dset = datasets.ZINC(data_path, subset=False, split='test')
    ...
    train_dset = GraphDataset_v2(train_dset, n_tags, degree=True)
    ...
    val_dset = GraphDataset_v2(val_dset, n_tags, degree=True)
    ...
    if args.pos_enc is not None:
        ...
        model = GraphTransformerGenGCN(in_size=input_size,
                                 nb_class=1,
                                 d_model=args.dim_hidden,
                                 dim_feedforward=2*args.dim_hidden,
                                 dropout=args.dropout,
    ...
    test_dset = GraphDataset_v2(test_dset, n_tags, degree=True)
