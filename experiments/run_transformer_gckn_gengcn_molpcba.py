...
def main():
    ...
    gckn_pos_enc_values = gckn_pos_encoder.apply_to(
        train_idx, dataset[torch.cat((valid_indices, test_indices), 0)], batch_size=64, n_tags=n_tags, ds_name=args.dataset)
    ...
    try:
        for epoch in range(args.epochs):
            ...
            train_loss, train_ap = train_epoch(model, train_loader, criterion, optimizer, lr_scheduler, epoch, args.use_cuda, nb_class=nb_class)
            val_loss, val_ap = eval_epoch(model, val_loader, criterion, args.use_cuda, nb_class=nb_class)
            test_loss, test_ap = eval_epoch(model, test_loader, criterion, args.use_cuda, nb_class=nb_class)
